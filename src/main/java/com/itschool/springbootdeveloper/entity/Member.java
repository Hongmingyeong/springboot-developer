...
// @AllArgsConstructor
@Getter
@Entity/*(name = "users")*/ // Member 객체를 JPA가 관리하는 엔티티로 지정, 속성 중에 name 값을 사용하면 name의 값을 가진 테이블 이름과 매필이 됨
public class Member {
    ...
    // SEQUENCE : 데이터베이스 시퀸스를 사용해서 기본키를 할당하는 방법, 오라클에서 주로 사용
    // TABLE : 키 생성 테이블 사용
    @Column(name = "id", updatable = false) // DB 실제 컬럼명, name을 넣지 않으면 변수명을 lower_snake_case로 간주
    private Long id; // DB 테이블의 'id' 컬럼과 매칭

    @Column(nullable = false, columnDefinition = "VARCHER(20) DEFAULT ''") // name이라는 not column과 매핑
    // name : 필드와 매핑할 컬럼이름, 설정하지 않으면 필드 이름으로 지정해줌
    // nullable : 컬럼의 null 허용 여부, 설정하지 않으면 ture(nullable), false(NOT NULL)
    // unique : 컬럼의 유일한 값(unique 제약조건) 여부, 설정하지 않으면 false(non unique)
    // columnDefinition : 컬럼 정보 설정, DDL을 직접 정의하고 싶을 때
